...
app.get(routes.getItem, function (req, res) {
  res.send(
    JSON.stringify(itemsDb.value.filter((elem) => elem._id === req.params.id))
  );
});
...
app.post(routes.editComment, urlencodedParser, (req, res) => {
  ...
  const ourGood = itemsDb.value.filter(
    (elem) => elem._id === req.body.fetchData.id
  );
  const ourGoods = itemsDb.value.filter(
    (elem) => elem._id !== req.body.fetchData.id
  );
  ...
  console.log(
    (ourGood[0].comments = [
      ...ourGood[0]?.comments,
      { id: new Date().valueOf(), description: req.body.fetchData.title },
    ])
  );
